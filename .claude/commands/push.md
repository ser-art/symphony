...
2. Run the project's validation gate before pushing (e.g., `make -C elixir all`
   for the Elixir reference implementation, or whatever the project defines).
3. Push branch to `origin` with upstream tracking if needed, using whatever
   remote URL is already configured.
4. If push is not clean/rejected:
   - If the failure is a non-fast-forward or sync problem, run the `pull`
     command to merge `origin/main`, resolve conflicts, and rerun validation.
...
7. If a PR body checker is available (e.g., `mix pr_body.check` in the Elixir
   implementation), validate the PR body and fix all reported issues.
...
# Validation gate — adjust to your project's tooling
# Example for Elixir reference implementation: make -C elixir all

# Initial push: respect the current origin remote.
git push -u origin HEAD

# If that failed because the remote moved, use the pull command. After
# pull resolution and re-validation, retry the normal push:
git push -u origin HEAD
...
# If a PR body checker is available, validate:
# tmp_pr_body=$(mktemp)
...
- Distinguish sync problems from remote auth/permission problems:
  - Use the `pull` command for non-fast-forward or stale-branch issues.
