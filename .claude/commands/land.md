...
3. If the working tree has uncommitted changes, commit with the `commit` command
   and push with the `push` command before proceeding.
4. Check mergeability and conflicts against main.
5. If conflicts exist, use the `pull` command to fetch/merge `origin/main` and
   resolve conflicts, then use the `push` command to publish the updated branch.
6. Ensure review comments (if present) are acknowledged and any required
   fixes are handled before merging.
7. Watch checks until complete.
8. If checks fail, pull logs, fix the issue, commit with the `commit` command,
   push with the `push` command, and re-run checks.
...
if [ "$mergeable" = "CONFLICTING" ]; then
  # Run the `pull` command to handle fetch + merge + conflict resolution.
  # Then run the `push` command to publish the updated branch.
fi
...
# Wait for review feedback: agent reviews arrive as issue comments that start
...
> **Note:** `land_watch.py` lives at `.codex/skills/land/land_watch.py` and
> currently recognizes `[codex]` comment prefixes. A future enhancement will
> add `[claude]` prefix recognition.
...
- If checks fail, pull details with `gh pr checks` and `gh run view --log`, then
  fix locally, commit with the `commit` command, push with the `push` command,
