...
- Primary runtime log: `log/symphony.log`
  - Contains orchestrator, agent runner, and session lifecycle logs.
...
`elixir/docs/logging.md` requires these fields for issue/session lifecycle logs.
...
2. Establish timeline:
    ...
    - Follow with `session completed`, `ended with error`, or worker exit
      lines.
3. Classify the problem:
    - Stall loop: `Issue stalled ... restarting with backoff`.
    - App-server startup: `session failed ...`.
...
Session diagnostics are emitted into `log/symphony.log` and keyed by
