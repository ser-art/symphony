...
Use this command for raw Linear GraphQL work during Symphony sessions.

## Tool Availability

The `linear_graphql` tool is exposed by Symphony's Codex app-server session and
reuses Symphony's configured Linear auth. If you are running as Claude Code
outside an app-server session, use one of:

- **Linear MCP server** — if configured, use the Linear MCP tools directly.
- **Linear API token** — set `LINEAR_API_KEY` and use `curl` or a GraphQL
  client against `https://api.linear.app/graphql`.

The GraphQL queries and mutations below work identically regardless of the
transport method.
...
- Once you have the internal issue id, prefer `issue(id: $id)` for narrower
  reads.
...
1. Call `fileUpload` to get `uploadUrl`, `assetUrl`, and any required upload
   headers.
